...
export const reportController = {
  uploadReport: asyncHandler(async (req, res) => {
    ...
    const report = await reportService.uploadReport( req.user.id, req.file, report_name, report_type);
    // Notify user
    await notificationService.createInAppNotification( req.user.id, 'Report Uploaded',`Your report "${report_name}" has been uploaded and is being processed.`,'INFO');
  ...
  getUserReports: asyncHandler(async (req, res) => {
    ...
    return ApiResponse.paginated(res, reports, total, page, limit, 'Reports fetched successfully');
  }),
